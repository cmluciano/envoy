...
createTransportSocketFactoryHelper(const Protobuf::Message& message, bool is_upstream) {
  ...
  HandshakerFactory factory =
    ...
    //
    // grpc_pollset_set adds pollsets of interested parties & automatically adds their registered fd's
    // hard-coding this to nullptr results in creation of a dedicated tsi_thread
    ...
    tsi_result status = alts_tsi_handshaker_create(
        options.get(), target_name, handshaker_service.c_str(), is_upstream, interested_parties, &handshaker);
